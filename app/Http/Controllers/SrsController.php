<?php

namespace App\Http\Controllers;

use App\Models\SrsCard;
use App\Models\StudyCardSet;
use App\Models\StudyCard;
use App\Models\UserGamification;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Carbon\Carbon;

class SrsController extends Controller
{
    /**
     * Mostrar la vista principal de Repetición Espaciada.
     */
    public function index()
    {
        $sets = collect();
        $setsData = collect();

        if (Auth::check()) {
            $sets = StudyCardSet::where('user_id', Auth::id())
                ->where('status', 'activo')
                ->with('cards')
                ->latest()
                ->get();

            $setsData = $sets->map(function ($set) {
                // Contar tarjetas pendientes de revisión para este set
                $dueCount = SrsCard::where('user_id', Auth::id())
                    ->where('study_card_set_id', $set->id)
                    ->where('next_review_at', '<=', Carbon::now())
                    ->count();

                $totalSrs = SrsCard::where('user_id', Auth::id())
                    ->where('study_card_set_id', $set->id)
                    ->count();

                return [
                    'id'          => $set->id,
                    'title'       => $set->title,
                    'cards_count' => $set->cards->count(),
                    'due_count'   => $dueCount,
                    'total_srs'   => $totalSrs,
                    'created_at'  => $set->created_at->toJSON(),
                ];
            })->values();
        }

        return view('repeticion-espaciada', compact('sets', 'setsData'));
    }

    /**
     * Obtener la cola de revisión de hoy para un set específico.
     */
    public function getReviewQueue(Request $request, $id)
    {
        $set = StudyCardSet::where('id', $id)
            ->where('user_id', Auth::id())
            ->with('cards')
            ->firstOrFail();

        // Tarjetas pendientes de revisión (next_review_at <= ahora)
        $dueCards = SrsCard::where('user_id', Auth::id())
            ->where('study_card_set_id', $set->id)
            ->where('next_review_at', '<=', Carbon::now())
            ->orderBy('next_review_at', 'asc')
            ->get();

        // Mapear con los datos de la tarjeta original
        $cards = $dueCards->map(function ($srsCard) {
            $card = $srsCard->studyCard;
            return [
                'srs_id'       => $srsCard->id,
                'card_id'      => $card->id,
                'card_index'   => $srsCard->card_index,
                'front'        => $card->front,
                'back'         => $card->back,
                'ease_factor'  => $srsCard->ease_factor,
                'interval_days'=> $srsCard->interval_days,
                'repetitions'  => $srsCard->repetitions,
                'next_review'  => $srsCard->next_review_at->toJSON(),
                'last_reviewed'=> $srsCard->last_reviewed_at?->toJSON(),
            ];
        });

        // Estadísticas
        $totalInSet = SrsCard::where('user_id', Auth::id())
            ->where('study_card_set_id', $set->id)
            ->count();

        $masteredCount = SrsCard::where('user_id', Auth::id())
            ->where('study_card_set_id', $set->id)
            ->where('interval_days', '>=', 21)
            ->count();

        return response()->json([
            'success' => true,
            'set' => [
                'id'    => $set->id,
                'title' => $set->title,
            ],
            'due_cards'    => $cards->values(),
            'total_due'    => $cards->count(),
            'total_cards'  => $totalInSet,
            'mastered'     => $masteredCount,
        ]);
    }

    /**
     * Sincronizar / inicializar datos SRS para un set.
     * Crea registros srs_cards para todas las tarjetas del set que aún no tengan registro.
     */
    public function sync(Request $request)
    {
        $request->validate([
            'set_id' => 'required|exists:study_card_sets,id',
        ]);

        $set = StudyCardSet::where('id', $request->set_id)
            ->where('user_id', Auth::id())
            ->with('cards')
            ->firstOrFail();

        $created = 0;
        foreach ($set->cards as $index => $card) {
            $existing = SrsCard::where('user_id', Auth::id())
                ->where('study_card_set_id', $set->id)
                ->where('card_index', $index)
                ->first();

            if (!$existing) {
                SrsCard::create([
                    'user_id'           => Auth::id(),
                    'study_card_set_id' => $set->id,
                    'study_card_id'     => $card->id,
                    'card_index'        => $index,
                    'ease_factor'       => 2.5,
                    'interval_days'     => 1,
                    'repetitions'       => 0,
                    'next_review_at'    => Carbon::now(), // Disponible inmediatamente
                    'last_reviewed_at'  => null,
                ]);
                $created++;
            }
        }

        return response()->json([
            'success' => true,
            'message' => "SRS inicializado: $created tarjetas registradas.",
            'created' => $created,
        ]);
    }

    /**
     * Registrar una revisión de tarjeta (algoritmo SM-2).
     *
     * quality: 0 = Again, 1 = Hard, 2 = Good, 3 = Easy
     */
    public function review(Request $request, $id)
    {
        $request->validate([
            'card_index' => 'required|integer|min:0',
            'quality'    => 'required|integer|in:0,1,2,3',
        ]);

        $srsCard = SrsCard::where('user_id', Auth::id())
            ->where('study_card_set_id', $id)
            ->where('card_index', $request->card_index)
            ->firstOrFail();

        // Mapear quality (0-3) a escala SM-2 (0-5)
        $qualityMap = [
            0 => 1,  // Again → 1 (reintento completo)
            1 => 3,  // Hard → 3
            2 => 4,  // Good → 4
            3 => 5,  // Easy → 5
        ];
        $q = $qualityMap[$request->quality];

        // ── Algoritmo SM-2 ──────────────────────────────────────
        $easeFactor = $srsCard->ease_factor;
        $interval   = $srsCard->interval_days;
        $reps       = $srsCard->repetitions;

        if ($q < 3) {
            // Fallo: resetear
            $reps = 0;
            $interval = 1;
        } else {
            // Éxito: incrementar
            $reps += 1;
            if ($reps === 1) {
                $interval = 1;
            } elseif ($reps === 2) {
                $interval = 6;
            } else {
                $interval = (int) round($interval * $easeFactor);
            }
        }

        // Actualizar ease_factor
        $easeFactor = $easeFactor + (0.1 - (5 - $q) * (0.08 + (5 - $q) * 0.02));
        if ($easeFactor < 1.3) $easeFactor = 1.3;
        if ($easeFactor > 3.0) $easeFactor = 3.0;

        // ── Guardar ─────────────────────────────────────────────
        $srsCard->update([
            'ease_factor'      => round($easeFactor, 4),
            'interval_days'    => $interval,
            'repetitions'      => $reps,
            'next_review_at'   => Carbon::now()->addDays($interval),
            'last_reviewed_at' => Carbon::now(),
        ]);

        // ── Gamificación: asignar XP ─────────────────────────────
        $xpMap = [0 => 5, 1 => 8, 2 => 15, 3 => 20]; // Again/Hard/Good/Easy
        $xpEarned = $xpMap[$request->quality] ?? 10;
        $leveledUp = false;
        $newAchievements = [];
        $masteredNow = false;

        if ($interval >= 21) $masteredNow = true;

        $gamification = UserGamification::firstOrCreate(
            ['user_id' => Auth::id()],
            ['xp' => 0, 'level' => 1, 'current_streak' => 0]
        );
        $gamification->total_reviews += 1;
        if ($masteredNow) $gamification->total_mastered += 1;
        $leveledUp = $gamification->addXp($xpEarned);
        $gamification->updateStreak();
        $newAchievements = $gamification->checkAchievements();

        return response()->json([
            'success' => true,
            'next_review' => $srsCard->next_review_at->toJSON(),
            'interval_days' => $interval,
            'ease_factor' => round($easeFactor, 2),
            'repetitions' => $reps,
            'xp_earned' => $xpEarned,
            'leveled_up' => $leveledUp,
            'new_achievements' => $newAchievements,
            'gamification' => $gamification->toArray(),
        ]);
    }

    /**
     * Obtener estadísticas de SRS del usuario.
     */
    public function stats()
    {
        $userId = Auth::id();

        $totalCards = SrsCard::where('user_id', $userId)->count();
        $dueNow    = SrsCard::where('user_id', $userId)
            ->where('next_review_at', '<=', Carbon::now())
            ->count();
        $mastered  = SrsCard::where('user_id', $userId)
            ->where('interval_days', '>=', 21)
            ->count();
        $learning  = SrsCard::where('user_id', $userId)
            ->where('interval_days', '<', 21)
            ->where('repetitions', '>', 0)
            ->count();
        $newCards  = SrsCard::where('user_id', $userId)
            ->where('repetitions', 0)
            ->count();

        return response()->json([
            'success' => true,
            'total_cards' => $totalCards,
            'due_now'     => $dueNow,
            'mastered'    => $mastered,
            'learning'    => $learning,
            'new_cards'   => $newCards,
        ]);
    }
}
